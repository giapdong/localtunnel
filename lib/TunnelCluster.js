...
module.exports = class TunnelCluster extends EventEmitter {
  ...
  open() {
	...
	const sessionTimeout = opt.session_timeout;
    ...
    const connLocal = () => {
	  ...
	  if (sessionConnection) {
		local.setTimeout(sessionTimeout * 1000);
		local.once('timeout', function() {
			debug('local connection with session-based timeout with connecting:', connecting);
			if (connecting) {
			} else {
				remote.removeListener('close', remoteClose);
				...
				setTimeout(function() {
					remote.end();
					remoteClose();
				}, Math.random() * sessionTimeout * 1000 + 1000);
			}
		});
	  }
